...
class App extends Component {
  ...
  listRecipes = function (list) {
    return list.map(item => <li key={item.id} onClick={this.setSelected}>{item.name}</li>);

  }

  setSelected = (event) => {
    const value = event.target.innerText;
    this.state.recipeItem.forEach((item) => {
      if (item.name === value) {
        this.setState({
          selected: item
        });
    ...
    return (
      <div className="App">
        <header className="App-header">
          ...
          <ul id="theList">{this.listRecipes(this.state.recipeItem)}</ul>
          <Recipe item={this.state.selected} />
          <Ingredients item={this.state.selected} />
        </header>
      </div>
    );
  }
